...
st.sidebar.title("📤 Upload Your Shopify CSV")
uploaded_file = st.sidebar.file_uploader("File must have: Date, SKU, Quantity, Revenue, Current_Stock", type=['csv'])

if uploaded_file is None:
    st.info("Using sample data. Upload your CSV to see real forecasts.")
    df = load_sample()
else:
...
with tab3:
    ...
    # --- DOWNLOAD BUTTON ---
    st.download_button(
        label="📥 Download Full SKU Report (CSV)",
        data=sku_data.to_csv(index=False).encode('utf-8'),
